...
window.onload = async function() {
    ...
    closeUploadModalBtn.onclick = () => adminUploadModal.style.display = "none";
    ...
    async function cargarMateriasYArchivos() {
        ...
        accordionContainer.innerHTML = "<p style='color: var(--caribbean-green);'>Consultando repositorio...</p>";
        ...
        try {
            ...
            for (const mat of materias) {
                ...
                tarjetaMateria.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(0,0,0,0.05); padding-bottom:5px;">
                        <h3>${mat.nombre}</h3>
                ...
                if (archivos && archivos.length > 0) {
                    archivos.forEach(file => {
                        const accionesAdmin = usuarioLogueado 
                            ? `<a href="${file.url_archivo}" download="${file.nombre_archivo}" target="_blank" class="view-link" style="background-color: var(--bangladesh-green); color: white; text-decoration: none; margin-right: 5px;">Descargar</a>
    ...
    window.onclick = (e) => {
        ...
        if (e.target === adminUploadModal) adminUploadModal.style.display = "none";
